<template>
  ...
  <el-row :gutter="20">
    ...
    <el-col :span="16"><div class="grid-content bg-purple"></div>
      ...
      <el-select id="test" v-model="form.wx_id" placeholder="名字需先在小程序保存信息">
      ...
      <el-button @click="selectall()">查询</el-button>
        <el-button @click="resetAllrule()">重置搜索</el-button>
        <el-button class="order" type="button" @click="dialogFormVisible = true" >新增个人用餐数据</el-button>
        ...
        <el-dialog title="新增预约用餐" :visible.sync="dialogFormVisible">
          <el-form :model="form" size="small">
            ...
            <el-form-item label="用餐时间">
              <el-date-picker type="date" placeholder="选择日期" v-model="form.appointment_at" value-format="yyyy-MM-dd" :picker-options="pickerOptions"></el-date-picker>
            </el-form-item>
            <el-form-item label="餐型">
              <el-select id="test" v-model="form.meal_type" placeholder="请选择餐型">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              ...
              <span v-else><i class="el-icon-close" @click="bookamealbreak()" style="color: red;font-size: 20px"></i> </span>
            </template>
          </el-table-column>
          <el-table-column
              prop="meal_type"
            ...
            <template slot-scope="scope">
              ...
              <span v-else><i class="el-icon-close" @click="bookameallunch()" style="color: red;font-size: 20px"></i>
                </span>
            </template>
          </el-table-column>
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <span v-else><i class="el-icon-close" @click="bookamealdinner()" style="color: red;font-size: 20px"></i> </span>
            </template>
          </el-table-column>
        </el-table>
      </template>
  ...
  methods: {
    ...
    oktheorder() {
      service.post("order/order",{
        ...
        'appointment_at':this.form.appointment_at,
        ...
        if(res.code == 0) {
          ...
          this.this.$message.error(res.msg);
        }
    ...
    cancelmealsone(mealtypeId) {
      ...
      this.$confirm('此操作将取消当日此用户的早餐, 是否继续?', '提示', {
        ...
        if(this.typesss[0] == 0 ) {
          ...
          alert("由于当日数据已经导出。为了确保数据准确性。请重新导出数据！！");
    ...
    cancelmealstwo(mealtypeId) {
      this.$confirm('此操作将取消当日此用户的午餐, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
    ...
    cancelmealstrr(mealtypeId) {
      this.$confirm('此操作将取消当日此用户的晚餐, 是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
    ...
    bookamealbreak() {
      //新增早餐
      alert("新增他人早餐 请点击上方新增用餐按钮！！");
    },
    bookameallunch() {
      //新增午餐
      alert("新增他人午餐 请点击上方新增用餐按钮！！");
    },
    bookamealdinner() {
      //新增晚餐
      alert("新增他人晚餐 请点击上方新增用餐按钮！！");
    },
    selectall() {
      this.test();
      let params = {
